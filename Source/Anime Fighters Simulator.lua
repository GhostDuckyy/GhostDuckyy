...
do
    task.spawn(function()
        assert(isfile, "Required 'isfile' Function")
        assert(isfolder, "Required 'isfolder' Function")
        assert(makefolder, "Required 'makefolder' Function")
        assert(writefile, "Required 'writefile' Function")
        assert(delfile, "Required 'delfile' Function")
        assert(listfiles, "Required 'listfiles' Function")
        assert(appendfile, "Required 'appendfile' Function")

        if not isfolder("GhostyDuckyy") then
            makefolder("GhostyDuckyy")
        end
        ...
        local function refreshConfig()
            local path = "GhostyDuckyy"
            local list = listfiles(path)
            local t = {}

            for _, v in next, list do
                local name = string.split(string.sub(v, 14), ".txt")[1]
                table.insert(t, tostring(name))
            end
            return t
        end

        local config = w:NewTab("Config")
        local setting = config:NewSection("Setting")

        setting:NewTextBox("File name", "Set file name", function(v)
            if type(v) ~= "string" then tostring(v) end
            getgenv().Config.FileName = "GhostyDuckyy/"..v..".txt"
        end)

        setting:NewButton("Save config", "Save config to folder",function()
            local a = refreshConfig()

            if rawget(a, getgenv().Config.FileName) then
                appendfile(getgenv().Config.FileName, "abc")
            else
                writefile(getgenv().Config.FileName, "a")
            end
        end)

        local ConfigDropdown = setting:NewDropdown("Config List", "Select config to save/load", {}, function(v)
            getgenv().Config.SelectedFile = "GhostyDuckyy/"..tostring(v)..".txt"
        end)

        setting:NewButton("Refresh", "Refresh file list", function()
            local a = refreshConfig()
            ConfigDropdown:Refresh(a)
        end)
    end)
